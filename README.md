...
This repository contains estimates of the time-varying reproduction number for every country in the world listed in the ECDC Covid-19 data source and subnational estimates for 9 countries. Summarised estimates can be found in `data/results/national/cases/summary` and `data/results/national/deaths/summary` (based on cases and deaths respectively). Estimates for each country can be found in `data/results/national/cases/national` and `data/results/national/deaths/national`. Subnational estimates can be found in the relevant country folder (`data/results/subnational/country`) with the same folder structure as for the national estimates. Estimates are generated using [`{EpiNow2}`](https://epiforecasts.io/EpiNow2/) and presented on [epiforecasts.io/covid](https://epiforecasts.io/covid) (which also outlines the method used). An [interactive visualisation](https://hamishgibbs.github.io/rt_vis/) is currently under development.

WARNING: We are in a transition phase - the results can be generated in two locations. Only results in /data/results will be committed and production will shift to this location. Data will be output to this location by using the `-d` flag. For those who require data in the old location you can (for the time being) run without the -d flag. This will be deprecated!
